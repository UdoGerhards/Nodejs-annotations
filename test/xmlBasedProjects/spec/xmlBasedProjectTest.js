...
describe("XML based context annotation test suite", function(){
    ...
    it('Should instantiate a type safe project', function() {
        ...
        /*
            Initialize context
         */
        var contextRoot = resourcesPath+path.sep+path.join("typeSafe");
        var contextInfo = {
            "scan": [
                contextRoot
            ]
        ...
        return new Promise(function(resolve, reject){
            factory.on(global.phase._FINAL_APPLICATION_CONTEXT_, function(applicationStack) {
                ...
                try {
                    ...
                    assert.typeOf(context.typeBean._number, "Number", "TypeBean 'nunber' is not a number");

                    context.typeBean.setNumber("1000");
                    assert.typeOf(context.typeBean._number, "Number", "TypeBean 'nunber' is not a number");

                    context.typeBean._integer = 10000.23455;
                    assert.typeOf(context.typeBean._integer, "Number", "TypeBean 'nunber' is not a number / integer");

                    context.typeBean.setInteger("1000");
                    assert.typeOf(context.typeBean._number, "Number", "TypeBean 'nunber' is not a number / integer");

                    context.typeBean._float = "10000.23455";
                    assert.typeOf(context.typeBean._float, "Number", "TypeBean 'nunber' is not a number / float");

                    context.typeBean.setFloat("1000.2345");
                    assert.typeOf(context.typeBean._number, "Number", "TypeBean 'nunber' is not a number / float");

                    context.typeBean._boolean = "true";
                    assert.typeOf(context.typeBean._boolean, "Boolean", "TypeBean 'boolean' is not a boolean");
                    ...
                    assert.typeOf(context.typeBean._string, "String", "TypeBean 'string' is not a string");
