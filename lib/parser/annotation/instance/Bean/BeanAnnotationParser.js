...
class BeanParser extends BaseParser {
    ...
    _stage(annotationInformation, beanStructure, applicationStructure) {
        ...
        let leadingToken = annotationInformation.tokens[0];
        let parser = instance.tokenParser[leadingToken.type];

        let result = null;
        try {
            result = parser.parse(leadingToken);
        } catch(e) {
            console.log(util.inspect(leadingToken, {depth:25}));
        }
        ...
        let paramText = annotationInformation.annotationValue;

        let parameterMatch = new RegExp("\([\"|\'](.*)[\"|\']\)", "g");;
        let match = parameterMatch.exec(paramText);
       ...
       // if (debug)
        ...
        if (namespace.length === 1) {
            ...
            beanStructure.loader = loader;
                ...
                let parentBeanStructure = instance._getSingleBeanStructureFromApplicationStack(namespaceSeq, applicationStructure, debug);
