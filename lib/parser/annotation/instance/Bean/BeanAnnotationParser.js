...
class BeanParser extends BaseParser {
     ...
     * @returns {*}
     */
    parseXML(currentTag, propertyStack, context, stack, beanStructure) {
        let instance = this;
        ...
        let annotationInformation = {
            ...
            annotationValue: "(\""+id+"\")",
            comment: instance.annotation.name+"(\""+id+"\")",
        };

        /* Create bean structure */
        beanStructure = {
        ...
        /* Process possible attributes and add them as child */
        if (namespace) {
            let annotationParser = instance.annotationParser["Namespace"];
            if (annotationParser) {
                let child = instance.parseXML(currentTag, propertyStack, context, stack, beanStructure);
                if (child) {
                    childs.push(child);
                }
            }
        ...
        if (prototype) {
            var annotationParser = instance.annotationParser["Prototype"];
            if (annotationParser) {
                let child = instance.parseXML(currentTag, propertyStack, context, stack, beanStructure);
                if (child) {
                    childs.push(child);
        ...
        /* Add childs to annotation informatio if there are childs */
        if (childs) {
            annotationInformation.childs = childs;
        }
