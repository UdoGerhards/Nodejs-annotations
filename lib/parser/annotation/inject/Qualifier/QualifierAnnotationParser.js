...
class QualifierParser extends BaseParser {
    ...
    parseXML(currentTag, propertyStack, context, stack, beanStructure) {
        ...
        if (currentTag.name == "Propoerty") {
            let propertyMapper = instance.xmlMapper["Property"];
            beanStructure = propertyMapper.parseXML(currentTag, annotation, beanStructure);

            let valueMapper = instance.xmlMapper["Value"];
            beanStructure = valueMapper.parseXML(currentTag, annotation, beanStructure);
        }

        if (currentTag.name == "Reference") {
            let referenceMapper = instance.xmlMapper["Reference"];
            beanStructure = referenceMapper.parseXML(currentTag, annotation, beanStructure);
        }
