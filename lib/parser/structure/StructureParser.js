...
class StructureParser {
    ...
    parse(fileInfo) {
        ...
        var astTree = null;
        try {
            astTree = esprima.parse(data, {sourceType: 'module', comment: true, loc: true, range: false}, function(node){
                if (!node.directive || (node.init && node.init == "ThisExpression")) {
    ...
    _cleanUp(structure) {
        ...
        for (let cleanIndex = 0; cleanIndex < structure.length; cleanIndex++) {
            if (structure[cleanIndex].property) {
                propertyStack.push(structure[cleanIndex].property);
                let stack = propertyStack;
                if (!structure[cleanIndex].prototypePath) {
                    structure[cleanIndex].prototypePath = propertyStack.slice(0, propertyStack.length);
                ...
                for (let expressionIndex = 0; expressionIndex < tokens.length; expressionIndex++) {
                    let expressionObject = tokens[expressionIndex];
                        ...
                        if (tokenParser) {
                            ...
                            struct = tokenParser.cleanUp(expressionObject, struct);
                        }
                        break;
                }
                ...
                for (let searchIndex = 0; searchIndex < searchArr.length; searchIndex++) {
                    ...
                    if (start >= beanStart && end <= beanEnd) {
                        for (let expressionIndex = 0; expressionIndex < tokens.length; expressionIndex++) {
                            ...
                            let tokenParser = instance.expressionParser[expressionObject.type];
                            if (tokenParser) {
                                annotation = tokenParser.cleanUp(expressionObject, annotation);
