...
var glob = require("glob")
    ...
    , Bean = require("../../annotation/instance/Bean/BeanAnnotationClass.js")
...
class StructureParser {
    ...
    _buildLevels(beanStack) {
        ...
        if (beanStack.length > 1) {
            ...
            for (var index = 1; index < beanStack.length; index++) {
                ...
                console.log(levelIncDecrementer);
                ...
                if (levelIncDecrementer) {
                    ...
                    if (processedBeans.length > 0) {
                        level = instance._getNextLevel(beanStack[index], processedBeans, level) + 1;
                    } else {
                        level = 1;
                    }

                    var tmpProcessed = [];
                    for (var processedBeanIndex = 0; processedBeanIndex < level; processedBeanIndex++) {
                        tmpProcessed.push(processedBeans[processedBeanIndex]);
                    }

                    processedBeans = tmpProcessed;

                    beanStack[index].level = level;           // Correction +1 since returned level can be 0
                    outBeanEnd = innerBeanEnd;
                }
            }
    ...
    _getNextLevel(bean, processedBeans, currentLevel) {
