...
class StructureParser {
    ...
    _cleanUp(structure) {
        ...
        for (let cleanIndex = 0; cleanIndex < structure.length; cleanIndex++) {
            ...
            if (structure[cleanIndex].descriptor) {
                ...
                if (structure[cleanIndex].descriptor.property) {
                    ...
                    if (cleanIndex > 0) {
                        ...
                        if (!Array.isArray(structure[cleanIndex-1].structure)) {
                            structure[cleanIndex-1].structure = [];
                        }
                        structure[cleanIndex-1].structure.push(qualifierAnnotation);
                    }

                    structure[cleanIndex].descriptor.prototypePath = null;
                    delete structure[cleanIndex].descriptor.prototypePath;
                }
            }
        }
    ...
    _analyzeStructure(comment, tokens, structure, tokenStruct) {
        ...
        for (let annoIndex = 0; annoIndex < comment.annotations.length; annoIndex++) {
            annotation = comment.annotations[annoIndex].annotation;
            if (annotation.TYPE & global.TYPE_ANNOTATION_BEAN_CONTEXT_AWARE) {
                ...
                if (!struct.descriptor) {
                    struct.descriptor = null;
                }

                if (!struct.descriptor.childs) {
                    struct.descriptor.childs = [];
                }
                struct.descriptor.childs.push(comment.annotations[annoIndex]);
