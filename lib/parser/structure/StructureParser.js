...
class StructureParser {
                ...
                if (commentStructure) {
                    ...
                    switch (parser.constructor.name) {
                        ...
                        case "Line":
                            ...
                            structure = instance._analyzeStructure(commentStructure, annotationTokensRaw, structure);
                            break;
                    }
                }
            }
    ...
    _analyzeStructure(comment, tokens, structure) {
        ...
        for (let annoIndex = 0; annoIndex < comment.annotations.length; annoIndex++) {
            ...
            if (annotation.TYPE && (annotation.TYPE & global.TYPE_ANNOTATION_BEAN_CONTEXT_AWARE)) {
                ...
                for (let expressionIndex = 0; expressionIndex < tokens.length; expressionIndex++) {
                    ...
                    if (expressionObject.type == "MemberExpression" || expressionObject.type == "ClassDeclaration" || expressionObject.type =="FunctionDeclaration" || expressionObject.type == "VariableDeclarator") {
                        struct.descriptor.tokens = [expressionObject];
                        break;
                    }
                }
                ...
                newBean = true;
            } else if (annotation.TYPE && (annotation.TYPE & global.TYPE_ANNOTATION_SUPPORT)) {
                if (!struct.childs) {
                    struct.childs = [];
                }
                struct.childs.push(annotation);
            }
        }
            ...
            for (let index=0; index<comment.annotations.length; index++) {
                ...
                for (let searchIndex = 0; searchIndex < searchArr.length; searchIndex++) {
                    ...
                    if (start >= beanStart && end <= beanEnd) {
                        if (!structure[searchIndex].structure) {
                            structure[searchIndex].structure = [];
                        }

                        for (let expressionIndex = 0; expressionIndex < tokens.length; expressionIndex++) {
                            let expressionObject = tokens[expressionIndex];
                            if (expressionObject.type == "MemberExpression" || expressionObject.type == "MethodDefinition" || expressionObject.type =="FunctionDeclaration") {
                                annotation.tokens = [expressionObject];
                                break;
                            }
                        }
                ...
                if (!added) {
                    ...
                    structure[0].structure.push(annotation);
