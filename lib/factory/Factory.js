...
`;
        ...
        for (var beanName in applicationStructure[0]["nameToNamespace"]) {
            ...
            if (type == "Resource" && skipExtResources) {
                continue;
            }
        ...
        for (var groupName in groups){
            ...
            fileContents +="together {\n";

            links[groupName] = [];
            group.forEach(function(type){

                if (objects[type]) {
                    objects[type].forEach(function (classObj) {
                        fileContents += classObj.def;
                        links[groupName].push(classObj.name);
                    });
            ...
            var before = null;
            for (var groupName in links){

                before = null;
                var counter = 0;
                links[groupName].forEach(function(val){

                    ++counter;
                    var separator = " -[hidden]> ";
                    if ((counter%colsPerGroup) == 0){
                        separator = " -[hidden]--> ";
                    }

                    if (before) {
                        linksString += before +separator+ val+"\n";
                        before = val;
                    } else {
                        before = val;
                    }
        ...
        for (var type in objects) {
            ...
            var nextType = objects[type];
            nextType.forEach(function(classDef){
                fileContents += classDef.def+"\n";
                last = classDef.name
            });
        }
