...
Factory.prototype._generateClassDiagramm = function (applicationStructure) {
    ...
    var defGroups = {
        configuration: ["Resource", "Properties", "Configuration", "Bean", "Service", "Controller", "Component", "Context", "Prototype" ]
    };
    ...
    if (projectLandScape) {
        ...
        var groups = projectLandScape.groups || defGroups;
        var colsPerGroup = projectLandScape.groupCols || 5;
        var skipExtResources = projectLandScape.skipExtResources || true;
...
`;
        ...
        for (var beanName in applicationStructure[0]["nameToNamespace"]) {
            ...
            classDef += "class " + className + singleton + " {\n";

            if (showBeanNames) {
                classDef += "  Bean: " + className + "\n..\n";
            }
            ...
            if (!applicationStructure[0]["nameToNamespace"][beanName]._isAspect) {
                ...
                properties.forEach(function (property) {
                    ...
                    if (property.startsWith("_")) {
                        ...
                        classDef += " " + visibility + property + "\n";
                    }

                });

                classDef += private.join("");
            ...
            objects[type].push(classObj);
        }

        fileContents += instance._layoutDiagram(groups, objects);

        //fileContents += linksString + before + " -[hidden]--> " + last + "\n";
            ...
            if (!prototypes.includes(beanName) && applicationStructure[0]["nameToNamespace"][beanName].structure) {
                ...
                structure.forEach(function (annotationInformation) {
                    ...
                    if (annotationInformation.injector) {
                        ...
                        //console.log(injectorClassStructure);
                        //console.log("\n");
...
Factory.prototype._layoutDiagram = function (groups, classDefsOrig) {

    var instance = this;
    var lastSection = "";

    var classDefs = classDefsOrig;
    var maxGroupClassCount = 0;

    var cols = 0;

    for(var type in classDefsOrig) {
        maxGroupClassCount = maxGroupClassCount < classDefsOrig[type].length?classDefsOrig[type].length: maxGroupClassCount;
    }


    var matrix = [];

    if (maxGroupClassCount > 0) {

        /*
         * Calculate max cols
         */
        cols = Math.ceil(Math.sqrt(maxGroupClassCount));
        cols = cols > 4 ? cols : 4;
        cols = Math.floor(cols * 1.6180);

        var row = [];

        var classDefsTmp = classDefsOrig;
        groups.configuration.forEach(function (group) {

            var section = classDefsTmp[group];
            var start = 0;
            if (section)
            while(start < section.length) {

                var nextSeq = start + cols;
                var col = section.slice(start, nextSeq);
                row.push(col);
                start = nextSeq;
            }

            classDefsOrig[group] = null;
            delete classDefsOrig[group];

        });

        var rest = [];
        for (var type in classDefsOrig) {
            rest = rest.concat(classDefsOrig[type]);
        }
        ...
        while(start < rest.length) {
            ...
            var nextSeq = start + cols;
            var col = rest.slice(start, nextSeq);
            row.push(col);
            start = nextSeq;
        }
        ...
        const DOWN = " -[hidden]--> ";
        var colLast = "";
        row.forEach(function(cols){

            if (colLast != "") {
                position += colLast + DOWN;
            }

            last = "";

            var rowContent = "together {\n";
            cols.forEach(function(entry){

                if (last != "") {
                    /*
                     * As long as max cols is not reached position right in same row
                     */
                    position += last + RIGHT + entry.name + "\n";
                    //colLast = last = entry.name;
                    last = entry.name;
                } else {
                    if (colLast != "") {
                        position += entry.name +"\n";
