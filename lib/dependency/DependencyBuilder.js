...
DependencyBuilder.prototype._reportModuleOrder = function(groupedDependencyPackages) {
    var instance = this;
    var logger = instance.logger;

    logger.info("---------------- Parsing order - modules, start ----------------");
    for (var module in groupedDependencyPackages) {
        var package = groupedDependencyPackages[module];
        logger.info("    => "+module);
        if (package.paths && package.paths.length > 0) {
            for (var index = 0 ; index < package.paths.length ; index++) {
                logger.debug("      " + util.inspect(package.paths[index]));
            }
        }
    }
    logger.info("---------------- Parsing order - modules, end ----------------");
...
DependencyBuilder.prototype.loadDependencies = function(globPatterns, excludes) {
    ...
    excludes.push("**/test/**/*.js");
    excludes.push("**/node_modules/**/*.js");
