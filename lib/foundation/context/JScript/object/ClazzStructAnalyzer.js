        ...
        return new Promise(function (resolve, reject) {
            ...
            try {
                astTree = esprima.parse(data, {
                    ...
                    if (node.trailingComments) {
                        ...
                        let mainAnnotationWait = new Promise(resolve => {
                            ...
                            if (!beanAnnotation && mainAnnotation instanceof instance.BeanAware) {
                                ...
                                instance.innerClassAnalyzeWait.push(innerClassAnalyzeWait);
                    ...
                    Promise.all(instance.innerClassAnalyzeWait).then(innerClazzMetaInformation => {
                        metaInformation.addInnerClazz(innerClazzMetaInformation);
                        i += 1;
                    }).then(res => {
                        if (i == instance.innerClazzMetaInformation.length) {
                            resolve(metaInformation);
                            // TODO: Dirty hack to execute resolve only once!
                            i = -1;
                        }
                    });
                });
