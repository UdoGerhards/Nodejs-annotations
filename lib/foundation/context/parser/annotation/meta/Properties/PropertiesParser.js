    ...
    _stage(annotationInformation, beanStructure, applicationStructure) {
        ...
        for (var index=0; index < annotationValue.length; index++) {
            ...
            if (scanPath.startsWith("../")) {
                ...
                scanPath = "/"+propPathSegments.splice(levelUpCount, propPathSegments.length).join("/");
                currentPwd = currentPathSegs.splice(0,currentPathSegs.length-levelUpCount).join("/");

                scanPath = currentPwd+scanPath;
            }
