...
class BeanParser extends BaseParser {
    ...
    constructor() {
        ...
        instance.QualifierAnnotation = null
    ...
    parseOpenXMLTag(moduleDir, currentTag, propertyStack, namespaceStack, applicationStack, beanStructure) {
        ...
        if (currentTag.attributes.src) {
            let clazz = null;
            let bean = require(currentTag.attributes.src);
            if (typeof bean == "object") {
                clazz = bean.constructor.name;
            } else {
                clazz = bean.name;
            }
            beanStructure.className = clazz;
        }

        /*
         * If there is no unique id, generate one
         */
