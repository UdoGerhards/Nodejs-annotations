...
class StructureParser {
    ...
    _analyzeStructure(comment, tokens, structure) {
        ...
        for (let index=0; index < comment.annotations.length; index++) {
            ...
            if (annotationObject instanceof(instance.contextAware)) {
                ...
                structure = instance._addSupport(annotationInformation, structure)
    ...
    _addSupport(annotation, structure) {

        let instance = this;
        let struct = {};

        for (let beanIndex=0; beanIndex < structure.length; beanIndex++) {
            ...
            structure[iIndex].childs.push(struct);
        ...
        for (let bIndex=1; bIndex < bStructure.length; bIndex++) {
            ...
            for(let aIndex = 0; aIndex < aStructure.length; aIndex++){
                if (aIndex != bIndex) {
                    ...
                    if (aBean.descriptor.start <= bBean.descriptor.start && aBean.descriptor.end >= bBean.descriptor.end) {
                        nameSpcArr.push(aBean.namespace);
                    } else {
                        break;
                    }
                }
            ...
            nameSpcArr.push(bBean.namespace);
            bStructure[bIndex].namespace = nameSpcArr.join(String.fromCharCode(12));
            nameSpcArr = [];
        }
