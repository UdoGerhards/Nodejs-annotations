...
class StructureParser {
            ...
            if (structure[cleanIndex].descriptor.property) {
                ...
                if (cleanIndex > 0) {
                    ...
                    let qualifierAnnotation = {
                        property: structure[cleanIndex].descriptor.property,
                        //prototypePath: propertyStack,
                        annotationValue: beanId,
                        annotation: instance.annotationParser["Qualifier"].annotation
                    };

                    structure[cleanIndex-1].structure.push(qualifierAnnotation);
                }
            }
        }
    ...
    _analyzeStructure(comment, tokens, structure, tokenStruct) {
        ...
        for (let annoIndex = 0; annoIndex < comment.annotations.length; annoIndex++) {
            ...
            if ((annotation.TYPE & global.TYPE_ANNOTATION_BEAN_CONTEXT_AWARE) ) {
            ...
            } else  if ((annotation.TYPE & global.TYPE_ANNOTATION_MODULE) ) {

                let moduleStruct = {};

                moduleStruct.annotation = comment.annotations[annoIndex];
                moduleStruct.start = span.loc.start.line;
                moduleStruct.end = span.loc.end.line;

                /*
                 moduleStruct.descriptor = comment.annotations[annoIndex];
                moduleStruct.codeFromTo = {
                    start: span.loc.start.line,
                    end: span.loc.end.line
                };
                moduleStruct.namespace = uuidV4();
                ...
                for (let expressionIndex = 0; expressionIndex < tokens.length; expressionIndex++) {
                    ...
                    if (tokenParser) {
                        ...
                        moduleStruct = tokenParser.getBeanContextAwareInformation(expressionObject, moduleStruct);
                    }
                    break;
                }
                                */

                moduleAware.push(comment.annotations[annoIndex]);
            }
        }
                        ...
                        if (tokenParser) {
                            ...
                            if (!annotation.prototypePath) {
                                ...
                                try {
                                    ...
                                    if (!Array.isArray(structure[index].structure)) {
