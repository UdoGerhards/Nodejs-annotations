...
class XMLContextBuilder {
            ...
            onopentag: function(tagname, attributes){
                ...
                switch(name) {
                    ...
                    case "bean":
                        ...
                        var singleton = attributes.singleton || true;
                        ...
                        var testMethod = attributes.test;
                    ...
                    case "parameter":
                        ...
                        var value = attributes.value;
                        //callFunc[callFunc.length-1].parameters.push(value);

                        var indexPath = currentID+"#"+propertyStack.join(".");
                        callFunc[indexPath].parameters.push(value);

                        break;

                    case "call":

                        var reference = currentTag.attributes.reference;
                        var method = currentTag.attributes.method;
    ...
    _processCallStack(context, callFunc) {
        var instance = this;

        for (var path in callFunc) {
            var method = callFunc[path].method;
            var parameters = callFunc[path].parameters;
            var reference = callFunc[path].reference;

            var indexPath = path.split("#");

            var res = context[reference][method].apply(context[reference], parameters);

            context[indexPath[0]][indexPath[1]] = res;
        }
    }
...
module.exports = exports = XMLContextBuilder;