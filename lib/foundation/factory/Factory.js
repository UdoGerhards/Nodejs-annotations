...
class Factory extends EventEmitter {
    ...
    bytesToSize(memObject, precision) {
        ...
        for (var name in memObject) {
            ...
            formated[name] = (input / Math.pow(1024, index)).toFixed(precision) +
                ' ' + unit[index] + 'B' + ' (' + input + ')';
        }
    ...
    async process(parameters) {
        ...
        if (parameters.file) {
            ...
            logger.debug("    using '" + parameters.file +
            ...
            return applicationContext.then(function (beanStructure) {
                ...
                logger.debug(
                    "    pre parsing of application context done ... coninuing with parsing ...  "
                );
                ...
                return res.then(function (applicationStack) {
                    ...
                    let cleanedStructure = instance.cleanApplicationStructure(
                        applicationStack);
                    cleanedStructure._parameters = parameters;

                    // Emit applicaton stack
                    instance.emit(global.phase._FINAL_APPLICATION_CONTEXT_,
                        cleanedStructure);
            ...
            // Scan for plugins
            var pluginCounter = instance._scanForPlugins(additionalPlugins,
                parameters);
            if (pluginCounter > 0) {
                instance.once(global.phase._BUILD_FINISHED_, function (pluginStack) {
                    ...
                    instance.once(global.phase._BUILD_FINISHED_, function (
                        applicationStack) {
                ...
                contextBuilder.once(global.phase._BUILD_FINISHED_, function (
                    applicationStack) {
                ...
                Promise.all(parsingPromises).then(function (result) {
                    ...
                    if (applicationStructure) {
                        ...
                        logger.error(
                            'Getting application context failed due "' + typeof applicationStructure +
                            '" parsing result! Did you specify an application context?'
                        );
                        rejectFunc(instance);
                    }
    ...
    _scanForPlugins(additionalPlugins, params) {
        ...
        var pluginPath = [process.env.PWD + instance.path.sep + "node_modules" +
      instance.path.sep + "@annotations" + instance.path.sep
    ];
        ...
        var searchInPath = Array.isArray(additionalPlugins) ? pluginPath.concat(
            additionalPlugins) : pluginPath;
    ...
    _parseFiles(parameters) {
        ...
        var externalContext = instance._fakeExternalResources(externalBeans) ||
            null;
        ...
        for (var scanIndex = 0; scanIndex < scanGlobs.length; scanIndex++) {
             ...
             * Load dependencies
             */
            var dependencyPackages = dependencyBuilder.loadDependencies(scan,
                scanExcludes);
            ...
            if (packageKeys.length > 0) {
                ...
                contextBuilder.parseFileInformation(dependencyPackages).then(function (
                    applicationStack) {
                    ...
                    let contextPromise = contextBuilder.processApplicationStack(
                        applicationStack);;
    ...
    cleanApplicationStructure(applicationStructure) {
        ...
        for (var beanName in nameToNamespace) {
            ...
            nameToNamespace[beanName].descriptor.annotationInformation =
