...
class DependencyBuilder {
    ...
    loadDependencies(globPatterns, excludes) {
        ...
        let filesSegregated = instance.filterJsonFiles(files);
        // let jsonFiles = filesSegregated.json;
        let filesToParse = filesSegregated.regular;
        let nodePackageJsons = filesSegregated.packageJson;
    ...
    filterJsonFiles(foundFiles) {
        ...
        foundFiles.forEach(function(value){

            if (value.endsWith("package.json")) {
                files.packageJson.push(value);
            } else if (value.endsWith(".json")) {
                //files.json.push(value);
            } else {
                files.regular.push(value);
            }
        });
    ...
    getPackageJsonDependencies(nodePackageJsons) {
        ...
        for (var index = 0 ; index < nodePackageJsons.length; index++) {
            ...
            var dependencies = nodePackageJson.dependencies ? Object.keys(nodePackageJson.dependencies):[];
...
export default new DependencyBuilder();